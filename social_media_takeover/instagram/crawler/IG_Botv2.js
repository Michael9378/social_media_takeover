...
function getPageInfo() {
    ...
    switch (pageType) {
        case 1:
            return getUserPageInfo();
        case 2:
            return getPostPageInfo();
        case 3:
            return getTagPageInfo();
        default:
            return null;
    }
}
...
function getPostPageInfo() {
    // bunch of userful stuff is stored here
    var shortcutInfo = window._sharedData.entry_data.PostPage;

    var postObj = {};
    if (typeof shortcutInfo == "undefined") {
        console.log("Instagram shared PostPage data wasn't loaded. Reload the page.");
        return null;
    }
    else {
        shortcutInfo = shortcutInfo[0].graphql.shortcode_media;
        postObj.id = location.href.split("/");
        postObj.id = postObj.id[postObj.id.length - 2];
        postObj.user = shortcutInfo.owner.username;
        postObj.time = shortcutInfo.taken_at_timestamp;
        postObj.description = shortcutInfo.edge_media_to_caption.edges[0].node.text;
        postObj.isVideo = shortcutInfo.is_video;
        postObj.num_likes = shortcutInfo.edge_media_preview_like.count;
        postObj.likes = shortcutInfo.edge_media_preview_like.edges
        postObj.comments = shortcutInfo.edge_media_to_comment.edges;
        postObj.tagged_users = shortcutInfo.edge_media_to_tagged_user.edges;
...
function getTagPageInfo() {
    ...
    if (typeof window._sharedData.entry_data.TagPage == "undefined") {
        console.log("Instagram shared TagPage data wasn't loaded. Reload the page.");
        return null;
    }
    else {
        var tagObj = {};
        var shortCut = window._sharedData.entry_data.TagPage[0].tag;
...
function getUserPageInfo() {
    ...
    if (typeof window._sharedData.entry_data.ProfilePage == "undefined") {
        console.log("Instagram shared TagPage data wasn't loaded. Loading data manually.");
        ...
        var shortcut = window._sharedData.entry_data.ProfilePage[0].user;
...
function parseForTags(string) {
    ...
    // loop through and split all peices on space and take first element
...
// scrapes followers and following and calls callback. Callback has 2 variables for followers and following
function getUserFollowBase(userObj, callback) {

    getUserFollowBaseHelper(userObj, 0, {}, function (results) {
        // close opened menu and wait a second then continue
        var closeBtn = getElementsByHtml("Button", "Close")[0];
        if (typeof closeBtn != "undefined")
            closeBtn.click();
        setTimeout(function () {
            getUserFollowBaseHelper(userObj, 1, results, function (results_2) { callback(results_2); });
        }, 250);
    });
}
...
function scrollBottom(container, old_li_count, li_matches, index, callback) {
    ...
    if( li_matches >=5 || li_count > 2000 || index > 500){
        ...
        index++;
        setTimeout(function () { scrollBottom(container, li_count, li_matches, index, callback); }, 200);
        return;
    }
}
...
function elementExists(selector, callback) {
    checkLoop(selector, callback, 0);
}

function checkLoop(selector, callback, index) {
    if (index > 50) {
        callback(true);
        return;
    }
    ...
    if (typeof $(selector)[0] != "undefined")
        ...
        setTimeout(function () { checkLoop(selector, callback, index) }, 100);
}
...
//    // used to see if we moved
//    // scroll down and load more
//    oldHeight = container.height();
//    container.scrollTop(oldHeight + 2000);
//    setTimeout(function () { container.scrollTop(oldHeight + 2000); }, 250);
//    setTimeout(function () { container.scrollTop(oldHeight + 2000); }, 500);
...
//        if (oldHeight == container.height() || loopCount > 100) {
//            // leave unbind command to be called multiple times for safe measure.
//            container.unbind('DOMSubtreeModified');
